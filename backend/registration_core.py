...
# -*- coding: utf-8 -*-
"""
Grok Register Web registration core.
Retains the browser/email/CPA workflow used by backend.job_manager.
"""
...
from backend import sso_to_auth_json as _s2cpa
from backend.email_providers import cloudflare as cloudflare_provider
from backend.email_providers import cloudmail as cloudmail_provider
from backend.email_providers import duckmail as duckmail_provider
from backend.email_providers import mailnest as mailnest_provider
from backend.email_providers import outlookemail as outlookemail_provider
from backend.email_providers import yyds as yyds_provider
from backend.email_providers.common import extract_verification_code as _extract_code
from backend.email_providers.common import generate_username as _generate_username
from backend.email_providers.common import pick_list_payload as _pick_list

from backend import browser_session as _bs
from backend import register_flow as _rf
from backend import connectivity as _conn
from backend.registration_store import RegistrationStore
from backend.proxy_utils import resolve_proxy_url
...
from backend.register_flow import (
    SIGNUP_URL,
...
_registration_store = None
_registration_store_lock = threading.Lock()
...
def get_registration_store():
    """懒加载 SQLite；首次启动时把旧账号 TXT 补录为成功结果。"""
    global _registration_store
    if _registration_store is not None:
        return _registration_store
    with _registration_store_lock:
        if _registration_store is None:
            store = RegistrationStore(RESULTS_DB_FILE)
            store.import_existing_accounts(ACCOUNTS_DIR)
            _registration_store = store
    return _registration_store
...
def email_registered_successfully(email):
    ...
    if not normalized:
        return False
    try:
        if get_registration_store().has_success(normalized):
            return True
    except Exception:
        pass
...
def persist_registration_result(
    *,
    ...
    try:
        return get_registration_store().add_result(
            {
                "batch_id": batch_id,
                "source": source,
...
def disable_outlookemail_after_cpa_success(email, cpa_detail=None, log_callback=None) -> dict:
    ...
    try:
        ...
        detail.update(
            ...
            disabled_at=RegistrationStore.now_text(),
            error="",
        )
        if log_callback:
