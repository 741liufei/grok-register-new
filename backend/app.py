# -*- coding: utf-8 -*-
"""FastAPI web console for account register / list / manage.

Reuses RegistrationStore + run_registration via backend.job_manager hooks.
"""
...
def create_app() -> FastAPI:
    ...
    def api_stats() -> Dict[str, Any]:
        gr = _gr()
        gr.load_config()
        store = gr.get_registration_store()
        return {"ok": True, "stats": store.stats(), "job": job_manager.status()}
    ...
    def api_account_auth_json(account_id: int, kind: str) -> Dict[str, Any]:
        ...
        gr = _gr()
        gr.load_config()
        rows = gr.get_registration_store().get_results_by_ids([account_id])
        if not rows:
            raise HTTPException(status_code=404, detail="记录不存在")
        try:
    ...
    def api_accounts_delete(body: DeleteAccountsBody) -> Dict[str, Any]:
        ...
        from backend.registration_files import (
    ...
    async def api_config_put(request: Request) -> Dict[str, Any]:
        if job_manager.status().get("running"):
    ...
    def api_job_status() -> Dict[str, Any]:
        return {"ok": True, "job": job_manager.status()}
    ...
    def api_job_logs(
        ...
        return {
            "ok": True,
            "logs": job_manager.get_logs(after_id=after_id, limit=limit),
    ...
    def api_job_start(body: StartJobBody) -> Dict[str, Any]:
        ...
        try:
            status = job_manager.start(count=count_i, workers=workers_i)
    ...
    def api_browser_kill_all() -> Dict[str, Any]:
        ...
        if job_manager.status().get("running"):
            try:
                ...
                pass
        try:
            result = gr._bs.kill_all_camoufox_processes(log_callback=job_manager._append_log)
        except Exception as exc:
            raise HTTPException(status_code=500, detail=f"终止浏览器失败: {exc}") from exc
        return {"ok": True, **result, "job": job_manager.status()}
